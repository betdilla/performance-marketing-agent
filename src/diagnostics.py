...
DECISION_BLOCKERS = {
    "LOW_DATA_CONFIDENCE": "SCALE",
    "ATTRIBUTION_DISCREPANCY": "SCALE",
    "CHEAP_CPA_WEAK_DOWNSTREAM": "SCALE",
}
...
def diagnose_case(campaign, creative, cohort, target_cpa=40.0):
    ...
    attribution_ratio = ratio(platform_roas, backend_roas)
    flags = []
    ...
    # Attribution is only escalated when both the absolute and relative gaps are material.
    # Tiny samples are kept separate because attribution variance is not decision-quality evidence.
    material_attribution_gap = attribution_gap >= 0.50 and attribution_ratio >= 1.50
    if conversions >= 20 and material_attribution_gap:
    ...
    blocked_decisions = sorted({DECISION_BLOCKERS[x] for x in flags if x in DECISION_BLOCKERS})
    ...
    return {
        ...
        "backend_roas": backend_roas,
        "ltv_cpa": round(ltv_cpa, 2),
        "repeat_rate": repeat_rate,
        "payback_days": payback,
        "ctr_change_pct": round(ctr_delta * 100, 1),
        "frequency_change_pct": round(frequency_delta * 100, 1),
        "downstream_conversion_change_pct": round(funnel_delta * 100, 1),
